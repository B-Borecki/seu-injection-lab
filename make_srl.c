...
__attribute__((section(".seu_section")))
static int32_t last_mz = 0;
__attribute__((section(".seu_section")))
static uint8_t srl_initialized = 0;

// Funkcja ograniczająca zmianę
static void srl_limit(int32_t *m, int32_t *last_m, const char *name) {
    int32_t diff = *m - *last_m;
    
    if (diff > MAX_CMD_STEP) {
        uart_puts("[SRL] ");
        uart_puts(name);
        uart_putdec_i32(*m);
        uart_puts(" -> ");
        *m = *last_m + MAX_CMD_STEP;
        uart_putdec_i32(*m);
        uart_puts("\r\n");
    } else if (diff < -MAX_CMD_STEP) {
        uart_puts("[SRL] ");
        uart_puts(name);
        uart_putdec_i32(*m);
        uart_puts(" -> ");
...
static void srl_protect(coil_cmd *cmd) {
    if (!srl_initialized) {
        last_mx = cmd->mx;
        last_my = cmd->my;
        last_mz = cmd->mz;
    ...
    srl_limit(&cmd->mx, &last_mx, "mx");
    srl_limit(&cmd->my, &last_my, "my");
    srl_limit(&cmd->mz, &last_mz, "mz");
}
...
static const int16_t sin_table[360] = {
    ...
    -173, -156, -139, -122, -105, -87, -70, -52, -35, -17, 0
};
...
static void task_sensor(void *arg) {
  ...
  while (1) {
    ...
    sample_sensor = (sensor_sample){
      .seq = seq,
      .bx  = BX + dx + variation,
      .by  = BY + dy + variation / 2,
      .bz  = BZ + dz + variation / 3,
    };
