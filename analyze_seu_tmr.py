...
RUN_LOGS_SEFI = {
    "curr_tmr": Path("seu_mode1_tmr.log"),
    "cmd_step": Path("seu_mode2_tmr.log"),
}
GDB_LOGS_SEFI = {
    "curr_tmr": Path("gdb_mode1_tmr.log"),
    "cmd_step": Path("gdb_mode2_tmr.log"),
}


OUTDIR = Path("plots_sefi")
...
def main():
    ...
    if not BASELINE_LOG.exists():
        raise SystemExit(f"ERROR: missing baseline log: {BASELINE_LOG}")
    ...
    for name, path in RUN_LOGS_SEFI.items():
        if not path.exists():
            raise SystemExit(f"ERROR: missing run log (SEFI): {path}")
        runs[name] = parse_act_samples(path)
    ...
    for name, path in GDB_LOGS_SEFI.items():
        if not path.exists():
            raise SystemExit(f"ERROR: missing gdb log (SEFI): {path}")
        flips[name] = parse_gdb_flips(path)
